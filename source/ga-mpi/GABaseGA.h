...
public:
  ...
  // Variables and methods to query and store mpi_tasks
  // and mpi_rank in the genetic algorithm class.
  int vmpi_rank;
  int vmpi_tasks;
  int mpi_rank() const {return vmpi_rank;}
  void mpi_rank(unsigned int x) {vmpi_rank=x;}
  int mpi_tasks() const {return vmpi_tasks;}
  void mpi_tasks(unsigned int x) {vmpi_tasks=x;}
